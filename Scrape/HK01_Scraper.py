...
with open('./ScrapedContents/url.csv','a+',encoding='utf-8') as f: #create a file to store the contents
    ...
    for item in href_lst: #iterate through the returned list
        ...
        if not(re.match('/[A-Za-z]*/[0-9]+/|/[A-Za-z...]|https://', item['href'])): 
            ...
            inlst.append([item.string, url]) #stores the sublist into a wholelist for actually writing in later
